...
function initObjectForDataset(dataset, scene, type, params, isStatic = false, date = null) {
  dataset.forEach(obj => {
    ...
    object = createCone(15, origin, end, 60, params.color);
    break;
...
function updateObjectsForDataset(dataset, dateInMilliseconds) {
  dataset.forEach(obj => {
    ...
    if (position) {
      if (obj.graphicalObject instanceof THREE.Mesh && obj.graphicalObject.geometry instanceof THREE.ConeGeometry) {
        ...
        const cone = createCone(40, origin, end, 95, obj.graphicalObject.material.color);
  ...
  console.log("End of updateObjectsForDataset");
}

// console.log("TOKEN 0");
// updateObjectsForDataset(updatedMessages, dateInMilliseconds); // Update Cones
// console.log("After updateObjectsForDataset call");
